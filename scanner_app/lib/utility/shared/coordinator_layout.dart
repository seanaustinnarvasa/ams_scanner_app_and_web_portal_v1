...
import 'package:nustar_turnstile_scanner/utility/shared/collapse_header.dart';
    ...
    return NotificationListener<ScrollNotification>(
      onNotification: (scrollNotification) {
        if (scrollNotification is ScrollEndNotification) {
          double range = (widget.header?.maxHeight)! - (widget.header?.minHeight ?? 0);
          if ((scrollController?.offset ?? 0) > 0 && (scrollController?.offset ?? 0) < range) {
            if ((scrollController?.offset ?? 0) < range / 2) {
              scrollController
                  ?.animateTo(0,
                      duration: Duration(milliseconds: 100), curve: Curves.ease)
                  .then((value) => scrollController?.jumpTo(0));
            } else if ((scrollController?.offset ?? 0) < range) {
              scrollController
                  ?.animateTo(range,
                      duration: Duration(milliseconds: 100), curve: Curves.ease)
